...
import models.{Customer, Address, AddressRepository, CustomerRepository}
...
@Singleton
class CustomerController @Inject()(customersRepo: CustomerRepository, addresses: AddressRepository,cc: MessagesControllerComponents)(implicit ec: ExecutionContext) extends MessagesAbstractController(cc) {
  ...
  def addCustomerHandle = Action.async { implicit request =>
    ...
    CustomerForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(
          BadRequest(views.html.customeradd(errorForm, categ))
        )
      },
      customer => {
  ...
  def updateCustomerHandle = Action.async { implicit request =>
    ...
    updateCustomerForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(
          BadRequest(views.html.customerupdate(errorForm,adrs))
        )
      },
      customer => {
  ...
  def updateCustomer(id: Int) = Action.async { implicit request =>
    ...
    CustomerForUpdate.map(customer => {
      ...
      Ok(views.html.customerupdate(prodForm,cat))
    })
    // update values
    // save
  ...
  def addCustomer() = Action.async { implicit request =>
    ...
    adr.map(c => Ok(views.html.customeradd(CustomerForm,c)))
