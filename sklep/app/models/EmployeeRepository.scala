...
@Singleton
class EmployeeRepository @Inject() (dbConfigProvider: DatabaseConfigProvider,val addressRepository: AddressRepository)(implicit ec: ExecutionContext) {
  ...
  class EmployeeTable(tag: Tag) extends Table[Employee](tag, "Employee") {
    ...
    def address_fk = foreignKey("address_fk", addressId, addressTable)(_.id, onDelete= ForeignKeyAction.Restrict)
